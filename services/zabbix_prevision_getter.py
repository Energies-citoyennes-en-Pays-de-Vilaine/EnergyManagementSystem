...
def get_zabbix_previsions(zabbix_key: str, table_name: str) -> None:
    ...
    data_to_post_to_database = [EMSPowerCurveData(t, v).get_create_or_update_in_table_str(table_name) for t, v in zip(data["timestamps"], data["values"])]
...
get_zabbix_previsions("Prevision_Prod_Enda_BGW_normale", "normal_solar_prediction")
